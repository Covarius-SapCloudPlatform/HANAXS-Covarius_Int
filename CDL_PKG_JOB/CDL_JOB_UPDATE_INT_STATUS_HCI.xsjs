	...
	function getAllDBEntries() {
		...
		var pstmtSrcKeys = conn.prepareStatement(
			"SELECT \"MESSAGE_GUID\", \"STATUS\" from \"CDL_SCH_LOGGING\".\"CDL_PULSE_LOG_MASTER\" WHERE \"STATUS\" NOT IN (\'COMPLETED\',\'FAILED\',\'RETRY\' ,\'ERROR\')"
		);
	...
	var dest = $.net.http.readDestination("CDL_PKG_LOGGING.CDL_PKG_JOB", "CDL_DESTINATION_HCI");
	...
	function retrieveSingleRecordFromHCI(guid) {
		...
		dest = $.net.http.readDestination("CDL_PKG_LOGGING.CDL_PKG_JOB", "CDL_DESTINATION_HCI");
	...
	function updateDBEntries(aRecordsToUpdate) {
		try {
			...
			var pstmt = conn.prepareStatement(
				"UPDATE \"CDL_SCH_LOGGING\".\"CDL_PULSE_LOG_MASTER\" SET STATUS = ? WHERE MESSAGE_GUID = ?");
