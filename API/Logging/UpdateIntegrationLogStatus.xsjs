(function() {
	...
	if ($.request.method === $.net.http.POST) {
		...
		$.response.setBody(JSON.stringify({
			message: "API Called",
			result: "POST is not supported, perform a GET to add Logging Entries"
		}));
	}
	// -------------------------------------------------------- // 
	...
	// ------------------------------------------------------------- //
	function retrieveSingleRecordFromHCI(guid, test) {
		//Instantiate the Connection Details to Cloud Integration
		dest = $.net.http.readDestination("Covarius_Int.API.Logging", "HCI");
	...
	function retrieveRecordsFromHCI(aDBRecords) {
		...
		for (var i = 0; i < aDBRecords.length; i++) {
			try {
				var oHCIRecord = retrieveSingleRecordFromHCI(aDBRecords[i].MESSAGE_GUID, "TEST");
	...
	function main() {
		...
		if ($.request.method === $.net.http.POST) {
			...
			$.response.setBody(JSON.stringify({
				message: "API Called",
				result: "POST is not supported, perform a GET to update Logging Entries"
			}));
		} else {
			try {
