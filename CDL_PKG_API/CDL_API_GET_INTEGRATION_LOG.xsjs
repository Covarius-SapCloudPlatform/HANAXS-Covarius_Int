(function() {
	...
	var gvStatusCode = $.request.parameters.get('statusCode');
	var gvStatusCodes;
	var gvErrorMessage;
	...
	//Check if there are multiple Status Code restrictions
	if (gvStatusCode) {
		if (gvStatusCode.indexOf(',') !== -1) {
			gvStatusCodes = gvStatusCode.split(',');
		}
	}
	// -------------------------------------------------------- // 
	...
	function _getLogEntries() {
		try {
			...
			if (!gvGuid) {
				...
				//StatusCode
				if (!gvStatusCodes && gvStatusCode) {
					lvQuery = lvQuery + ' AND STATUS_CODE = ' + "'" + gvStatusCode + "'";
				}

				if (gvStatusCodes) {
					for (var k = 0; k <= gvStatusCodes.length; k++) {
						if (gvStatusCodes[k]) {
							if (k === 0) {
								if (lvQuery.indexOf('WHERE') === -1) {
									lvQuery = lvQuery + ' WHERE STATUS_CODE = ' + "'" + gvStatusCodes[k] + "'";
								} else {
									lvQuery = lvQuery + ' AND STATUS_CODE = ' + "'" + gvStatusCodes[k] + "'";
								}

							} else {
								lvQuery = lvQuery + ' OR STATUS_CODE = ' + "'" + gvStatusCodes[k] + "'";
							}
						}
					}
				}
			...
			while (oResultSet.next()) {
				...
				var record = {
					...
					STATUS_CODE: oResultSet.getString(15),
					STATUS_MESSAGE: oResultSet.getString(16)
				};
